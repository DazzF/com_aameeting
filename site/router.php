...
/**
 * Routing class from com_aameetinglist
 *
 * @since  3.3
 */
class AameetinglistRouter extends JComponentRouterBase
{	
	/**
	 * Build the route for the com_aameetinglist component
	 *
	...
	public function build(&$query)
	{
		...
		if (isset($view) && isset($query['id']) && ($view === 'aameeting' || $view === 'aameetings'))
		{
			if ($mId != (int) $query['id'] || $mView != $view)
			{
				if (($view === 'aameeting' || $view === 'aameetings'))
				{
					$segments[] = $view;
					$id = explode(':', $query['id']);
					if (count($id) == 2)
	...
	{		
		...
		switch($segments[0])
		{
			...
			case 'aameetings':
				...
				{
					$id = $this->getVar('aameeting', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
	...
	protected function getVar($table, $where = null, $whereString = null, $what = null, $category = false, $operator = '=', $main = 'aameetinglist')
	{
...
function AameetinglistBuildRoute(&$query)
{
	$router = new AameetinglistRouter;
	
	return $router->build($query);
}

function AameetinglistParseRoute($segments)
{
	$router = new AameetinglistRouter;
