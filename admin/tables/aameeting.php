...
/**
 * Aameetings Table class
 */
class AameetinglistTableAameeting extends JTable
{
	/**
	...
	{
		parent::__construct('#__aameetinglist_aameeting', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_aameetinglist.aameeting'));
	}	
	...
	/**
	 * Overload the store method for the Aameeting table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New aameeting. A aameeting created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('aameeting', 'AameetinglistTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_AAMEETINGLIST_AAMEETING_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_aameetinglist.aameeting.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_aameetinglist.aameeting.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = NULL, $id = NULL) 
	{
		...
		$asset->loadByName('com_aameetinglist');
